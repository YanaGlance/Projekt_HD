...
SELECT
	  ...
	  ,(CASE WHEN Stage.VariableTarget LIKE 'AMR %' THEN 'AMR-perct'
                         ...
                         END) [VariableTarget]
	  ,TRY_CONVERT(NUMERIC(12, 6), REPLACE(REPLACE(REPLACE(REPLACE([Podgrupa], '%', ' '), ' ', '') , ',', '.'), ' ', '')) [Podgrupa]
      ,TRY_CONVERT(NUMERIC(12, 6), REPLACE(REPLACE(REPLACE(REPLACE([A16-49], '%', ' '), ' ', '') , ',', '.'), ' ', '')) [A16-49]
	  ,TRY_CONVERT(NUMERIC(12, 6), REPLACE(REPLACE(REPLACE(REPLACE([M16-49], '%', ' '), ' ', '') , ',', '.'), ' ', '')) [M16-49]
	  ,TRY_CONVERT(NUMERIC(12, 6), REPLACE(REPLACE(REPLACE(REPLACE([A4-15], '%', ' '), ' ', '') , ',', '.'), ' ', '')) [A4-15]
	  ,TRY_CONVERT(NUMERIC(12, 6), REPLACE(REPLACE(REPLACE(REPLACE([A4-9], '%', ' '), ' ', '') , ',', '.'), ' ', '')) [A4-9]
	  ,TRY_CONVERT(NUMERIC(12, 6), REPLACE(REPLACE(REPLACE(REPLACE([TotalIndividuals], '%', ' '), ' ', '') , ',', '.'), ' ', '')) [TotalIndividuals]
  FROM [dbo].[Stage] 
